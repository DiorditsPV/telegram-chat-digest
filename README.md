...
5. Недельный шардинг (`iso_week_of`, бакетизация, дозапись) — единый модуль `scripts/shards.py`; `tg_sync.py`, `import_export.py`, `migrate_to_weekly.py` импортируют его (`import shards`).
