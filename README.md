...
- `/tg-sync` — выгрузить новые сообщения и обновить `context.md` + `delta-log.md`,
  а также журнал решений/задач `decisions.jsonl` (тема · причина · следствие · статус).
...
- `/tg-topics` — доска решений и задач по темам (суть рабочих задач с одного взгляда,
  из `data/<name>/decisions.jsonl`).
- `/tg-why <тема>` — цепочка **причина → решение → следствие** по теме/решению с цитатами `#id`.
- `/tg-digest [дней]` — обзор движения решений/задач за период (новое/обновлённое/завершённое)
  и что **зависло** (open без движения), из `data/<name>/decisions.jsonl`.
- `/tg-mine [кто]` — активные решения/задачи на человеке («что на мне / на нём»); по умолчанию
  ответственный из поля `me:` в `config.yaml`.
